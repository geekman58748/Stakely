...
router.get("/config", auth_js_1.requireAuth, async (req, res) => {
    const { data: user } = await supabase_js_1.db.from("users").select("id").eq("wallet_address", req.walletAddress).single();
    if (!user) {
        res.status(404).json({ error: "User not registered" });
        return;
    }
    const { data } = await supabase_js_1.db.from("bot_configs").select("*").eq("user_id", user.id).single();
    res.json(data ?? null);
});
/** PUT /bots/config — upsert bot config */
router.put("/config", auth_js_1.requireAuth, async (req, res) => {
    const { data: user } = await supabase_js_1.db.from("users").select("id").eq("wallet_address", req.walletAddress).single();
    if (!user) {
        res.status(404).json({ error: "User not registered" });
        return;
    ...
    const { data, error } = await supabase_js_1.db.from("bot_configs").upsert({
        user_id: user.id,
...
router.post("/predict/:matchId", auth_js_1.requireAuth, async (req, res) => {
    ...
    const { data: config } = await supabase_js_1.db.from("bot_configs").select("*").eq("user_id", user.id).single();
    const archetype = (config?.archetype ?? "professor");
    const riskLevel = config?.risk_level ?? 5;
    // Fetch match + odds
    const [{ data: match }, odds] = await Promise.all([
        supabase_js_1.db.from("matches").select("*").eq("id", matchId).single(),
        txline_js_1.txline.getOdds(String(matchId)).catch(() => null),
    ]);
    if (!match) {
    ...
    // Store prediction
    const { data: stored } = await supabase_js_1.db.from("agent_predictions").insert({
        user_id: user.id,
        match_id: matchId,
        archetype,
        prediction,
...
router.get("/predictions", auth_js_1.requireAuth, async (req, res) => {
    const { data: user } = await supabase_js_1.db.from("users").select("id").eq("wallet_address", req.walletAddress).single();
    if (!user) {
        res.status(404).json({ error: "User not registered" });
        return;
    }
    const { data, error } = await supabase_js_1.db.from("agent_predictions")
