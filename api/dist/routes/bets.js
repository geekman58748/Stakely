...
async function getEscrow() {
    if (!_escrow)
        _escrow = await Promise.resolve().then(() => __importStar(require("../lib/escrow.js")));
    return _escrow;
}
...
router.get("/", auth_js_1.requireAuth, async (req, res) => {
    ...
    const { data: user } = await supabase_js_1.db.from("users").select("id").eq("wallet_address", req.walletAddress).single();
    if (!user) {
        res.status(404).json({ error: "User not registered" });
        return;
    }
    let q = supabase_js_1.db.from("bets").select(`
    *, 
...
router.post("/", auth_js_1.requireAuth, async (req, res) => {
    ...
    const { data: match } = await supabase_js_1.db.from("matches").select("id,kickoff_at,status").eq("id", match_id).single();
    if (!match) {
        res.status(404).json({ error: "Match not found" });
        return;
    }
    if (match.status === "finished" || match.status === "cancelled" || match.status === "postponed") {
        res.status(400).json({ error: "Match is already finished — no new bets accepted" });
        return;
    }
    // Live betting allowed — users can challenge friends mid-match
    // Get creator's user ID
    const { data: creator } = await supabase_js_1.db.from("users").select("id").eq("wallet_address", req.walletAddress).single();
    if (!creator) {
        res.status(404).json({ error: "User not registered. POST /users first." });
        return;
    }
    try {
    ...
    if (counterparty_wallet) {
        const { data: cp } = await supabase_js_1.db.from("users").select("id").eq("wallet_address", counterparty_wallet).single();
        if (!cp) {
            res.status(404).json({ error: "Counterparty wallet not registered" });
            return;
        }
        counterparty_id = cp.id;
    }
    const { data, error } = await supabase_js_1.db.from("bets").insert({
        id,
        match_id,
...
router.patch("/:id/accept", auth_js_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    const { accept_tx } = req.body;
    if (!accept_tx) {
        res.status(400).json({ error: "accept_tx is required" });
        return;
    }
    const { data: bet } = await supabase_js_1.db.from("bets").select("*").eq("id", id).single();
    if (!bet) {
        res.status(404).json({ error: "Bet not found" });
        return;
    }
    if (bet.status !== "challenged") {
        res.status(400).json({ error: `Cannot accept a bet with status: ${bet.status}` });
        return;
    }
    const { data: user } = await supabase_js_1.db.from("users").select("id").eq("wallet_address", req.walletAddress).single();
    if (!user) {
    ...
    if (user.id === bet.creator_id) {
        res.status(400).json({ error: "Cannot accept your own bet" });
        return;
    }
    if (bet.counterparty_id && bet.counterparty_id !== user.id) {
        res.status(403).json({ error: "This bet was sent to a specific counterparty" });
        return;
    }
    try {
    ...
    const { data, error } = await supabase_js_1.db.from("bets").update({
...
router.patch("/:id/cancel", auth_js_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    const { data: bet } = await supabase_js_1.db.from("bets").select("*").eq("id", id).single();
    if (!bet) {
        res.status(404).json({ error: "Bet not found" });
    ...
    const { data: user } = await supabase_js_1.db.from("users").select("id").eq("wallet_address", req.walletAddress).single();
    if (!user || user.id !== bet.creator_id) {
        res.status(403).json({ error: "Only the creator can cancel" });
        return;
    }
    if (!["challenged", "countered"].includes(bet.status)) {
        res.status(400).json({ error: "Can only cancel challenged or countered bets" });
        return;
    }
    const { data, error } = await supabase_js_1.db.from("bets")
...
router.post("/:id/settle", async (req, res) => {
    ...
    const { data: bet } = await supabase_js_1.db.from("bets").select("*, match:matches(result)").eq("id", id).single();
    if (!bet) {
        res.status(404).json({ error: "Bet not found" });
        return;
    }
    if (bet.status !== "locked" && bet.status !== "live") {
        res.status(400).json({ error: "Bet must be locked or live to settle" });
        return;
    }
    const result = bet.match?.result;
    if (!result) {
        res.status(400).json({ error: "Match result has not been verified" });
        return;
    }
    const winner_id = bet.creator_side === result ? bet.creator_id : bet.counterparty_id;
    if (!winner_id) {
        res.status(400).json({ error: "Bet has no funded counterparty" });
        return;
    }
    // Update bet
    ...
    await supabase_js_1.db.from("matches").update({ merkle_proof, merkle_stored_at: new Date().toISOString() }).eq("id", bet.match_id);
    // Update streaks + win/loss record
    if (winner_id) {
        const loserId = winner_id === bet.creator_id ? bet.counterparty_id : bet.creator_id;
        await Promise.all([
            supabase_js_1.db.rpc("increment_streak_win", { user_id: winner_id }),
            supabase_js_1.db.rpc("reset_streak_loss", { user_id: loserId }),
        ]);
    }
