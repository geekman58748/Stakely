...
router.post("/link-code", auth_js_1.requireAuth, async (req, res) => {
    const code = generateCode();
    await supabase_js_1.db.from("telegram_link_codes").insert({
        code,
...
router.post("/verify", async (req, res) => {
    ...
    if (!code || !telegram_id) {
        res.status(400).json({ error: "code and telegram_id required" });
        return;
    }
    const { data: linkCode } = await supabase_js_1.db.from("telegram_link_codes")
    ...
    await supabase_js_1.db.from("telegram_link_codes").update({ used_at: new Date().toISOString() }).eq("code", code.toUpperCase());
    ...
    (0, telegram_js_1.notifyLinkSuccess)(parseInt(telegram_id), data?.display_name ?? telegram_handle ?? "anon");
...
router.get("/user/:telegramId", async (req, res) => {
    const { data, error } = await supabase_js_1.db.from("users")
        ...
        .single();
    if (error || !data) {
        res.status(404).json({ error: "Telegram ID not linked" });
        return;
    }
    res.json(data);
});
...
router.post("/webhook", async (req, res) => {
    ...
    const teamName = (side, homeTeam, awayTeam) => side === "home" ? homeTeam : side === "away" ? awayTeam : "Draw";
    // ── /start ──────────────────────────────────────────────────────────────────
    if (command === "/start") {
        await tgSend(chatId, "hey! i'm Stakely \u{1F44B}\n\n" +
    ...
    if (command === "/link") {
        ...
        await supabase_js_1.db.from("telegram_link_codes").update({ used_at: new Date().toISOString() }).eq("code", code.toUpperCase());
        const { data: user, error } = await supabase_js_1.db.from("users")
    ...
    if (command === "/scores") {
        ...
        const lines = await Promise.all(liveMatches.map(async (m) => {
            try {
                const s = await txline_js_1.txline.getScore(m.id);
    ...
    if (command === "/bets") {
        ...
        const lines = bets.map(b => {
            const m = b.match;
            const name = b.creator?.display_name ?? "anon";
    ...
    if (command === "/mybets") {
        const { data: user } = await supabase_js_1.db.from("users").select("id").eq("telegram_id", fromId).single();
        if (!user) {
        ...
        const lines = bets.map(b => {
            const m = b.match;
            const cp = b.counterparty?.display_name ?? "waiting for opponent";
            const team = teamName(b.creator_side, m?.home_team, m?.away_team);
