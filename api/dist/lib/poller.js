...
const txline_js_1 = require("./txline.js");
const telegram_js_1 = require("./telegram.js");
// Lazy-load escrow to avoid blocking startup on Solana module load
let _settleOnChain = null;
async function getSettleOnChain() {
    if (!_settleOnChain)
        _settleOnChain = (await Promise.resolve().then(() => __importStar(require("./escrow.js")))).settleOnChain;
...
function mapGameState(gs) {
    ...
    if (["1h", "2h", "live", "inprogress", "in_progress", "in progress"].some(x => s.includes(x)))
        return "live";
    if (["ht", "halftime", "half_time", "half time"].some(x => s.includes(x)))
        return "halftime";
    if (["ft", "aet", "pen", "finished", "ended", "full time", "fulltime"].some(x => s.includes(x)))
        return "finished";
    if (["postponed", "cancelled", "abandoned"].some(x => s.includes(x)))
        return "postponed";
    return "scheduled";
}
async function tick() {
    try {
        ...
        const { data: liveMatches } = await supabase_js_1.db.from("matches")
        ...
        await Promise.all(liveMatches.map(async (match) => {
            let score;
            try {
                score = await txline_js_1.txline.getScore(match.id);
            }
            catch {
                return;
            }
            ...
            if (FINISH_STATES.has(score.status)) {
                ...
                await roastLosers(match.id, match.home_team, match.away_team, score.homeScore, score.awayScore, score.minute ?? undefined);
            }
        }));
...
async function settleBetsForMatch(matchId, homeTeam, awayTeam, homeScore, awayScore, merkleProof) {
    const result = homeScore > awayScore ? "home" : awayScore > homeScore ? "away" : "draw";
    const { data: bets } = await supabase_js_1.db.from("bets")
    ...
    await Promise.all(bets.map(async (bet) => {
        try {
            ...
            await supabase_js_1.db.from("bets").update({
                status: "settled",
                winner_id: winner.id,
                settle_tx,
            ...
            try {
                await supabase_js_1.db.rpc("increment_streak_win", { user_id: winner.id });
                await supabase_js_1.db.rpc("reset_streak_loss", { user_id: loser.id });
            }
            catch (rpcErr) {
            ...
            if (winner.telegram_id) {
                (0, telegram_js_1.notifySettlement)(winner.telegram_id, null, winner.display_name ?? "you", loser.display_name ?? "opponent", bet.amount_usdc, result === "home" ? homeTeam : result === "away" ? awayTeam : homeTeam).catch(() => null);
            }
            if (loser.telegram_id) {
                (0, telegram_js_1.notifyLosing)(loser.telegram_id, loser.display_name ?? "you", result === "home" ? awayTeam : result === "away" ? homeTeam : awayTeam, result === "home" ? homeTeam : result === "away" ? awayTeam : homeTeam, result === "home" ? awayScore : homeScore, result === "home" ? homeScore : awayScore, null).catch(() => null);
            }
...
async function roastLosers(matchId, homeTeam, awayTeam, homeScore, awayScore, minute) {
    const now = Date.now();
    const { data: bets } = await supabase_js_1.db.from("bets")
        ...
        if (homeScore !== awayScore) {
            ...
            if (loser?.telegram_id) {
                ...
                if (now - lastTime > ROAST_COOL_MS) {
                    ...
                    (0, telegram_js_1.notifyLosing)(loser.telegram_id, loser.display_name ?? "u", losingTeam, leadingTeam, homeScore < awayScore ? homeScore : awayScore, homeScore < awayScore ? awayScore : homeScore, minute ?? null).catch(() => null);
                }
            }
        }
    }));
...
async function pingUpcomingMatches() {
    try {
        ...
        const { data: upcoming } = await supabase_js_1.db.from("matches")
            .select("id,home_team,away_team,kickoff_at,pinged_soon")
            .eq("status", "scheduled").gte("kickoff_at", nowIso).lte("kickoff_at", hi);
        if (!upcoming?.length)
            return;
        const unpinged = (upcoming ?? []).filter((m) => !m.pinged_soon);
        if (!unpinged.length)
            return;
        const { data: users } = await supabase_js_1.db.from("users")
            .select("telegram_id").not("telegram_id", "is", null);
        if (!users?.length)
            return;
        for (const match of unpinged) {
            await supabase_js_1.db.from("matches").update({ pinged_soon: true }).eq("id", match.id);
            const minsUntil = Math.round((new Date(match.kickoff_at).getTime() - now) / 60000);
            await Promise.all(users
                .filter((u) => u.telegram_id !== null)
                .map(u => (0, telegram_js_1.notifyMatchSoon)(u.telegram_id, match.home_team, match.away_team, minsUntil)));
