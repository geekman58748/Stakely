...
exports.txlineMode = exports.txline = void 0;
...
class TxLineRealClient {
    ...
    async jwt() {
        ...
        if (!res.ok)
            throw new Error(`Failed to obtain guest JWT: ${res.status}`);
    ...
    async getFixtures(competitionId) {
        const params = competitionId ? { competitionId: String(competitionId) } : undefined;
        const response = await this.get("/api/fixtures/snapshot", params);
    ...
    async getOdds(fixtureId) {
        const response = await this.get(`/api/odds/snapshot/${fixtureId}`);
    ...
    async getScore(fixtureId) {
        const response = await this.get(`/api/scores/snapshot/${fixtureId}`);
        const records = normalizeRecords(response);
        ...
        const participant1Goals = firstNumber(statValues.get(1), latest.Participant1Goals, latest.participant1Goals, latest.HomeScore);
        const participant2Goals = firstNumber(statValues.get(2), latest.Participant2Goals, latest.participant2Goals, latest.AwayScore);
    ...
    async getSettlementProof(fixtureId, seq) {
        if (!Number.isInteger(seq) || seq <= 0)
            throw new Error("A real TxLINE score sequence is required");
...
function mapFixtureStatus(fixture) {
    const value = String(fixture.GameState ?? fixture.Status ?? fixture.FixtureStatus ?? "").toLowerCase();
    if (value.includes("postpon") || value.includes("cancel"))
        return "postponed";
    if (value.includes("finish") || value.includes("final") || value === "ft")
        return "finished";
    if (value.includes("half"))
        return "halftime";
    if (value.includes("live") || value.includes("progress") || value.includes("playing"))
        return "live";
    return "scheduled";
}
function mapScoreStatus(score) {
    ...
    if (action === "game_finalised" || value.includes("finish") || value === "ft" || period === 100)
        return "finished";
    if (value.includes("half"))
        return "halftime";
    if (value.includes("live") || value.includes("progress") || Number.isFinite(period))
        return "live";
    return "scheduled";
}
// ── Mock client ────────────────────────────────────────────────────────────────
class TxLineMockClient {
    async getFixtures() { return MOCK_FIXTURES; }
    async getOdds(id) {
        const map = {
...
exports.txline = useMock ? new TxLineMockClient() : new TxLineRealClient();
exports.txlineMode = useMock ? "mock" : "real";
